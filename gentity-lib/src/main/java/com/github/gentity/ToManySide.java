...
public abstract class ToManySide<T, C extends Collection<O>, O> extends RelationSide<T, O>{
	...
	private ToManySide(EntityStateProvider<T> esp, Function<T, C> collectionProvider) {
		super(esp);
		this.collectionProvider = collectionProvider;
	}
	
	public static <T,O> ToManySide<T, List<O>, O> of(EntityStateProvider<T> esp, Function<T, List<O>> listProvider, RelationSide<O, T> other) {
		ToManySide<T, List<O>, O> instance = of(esp, listProvider);
		instance.connect(other);
		return instance;
	}
	
	public static <T,O> ToManySide<T, List<O>, O> of(EntityStateProvider<T> esp, Function<T, List<O>> listProvider) {
		return new ToManySide<T, List<O>, O>(esp, listProvider) {
			@Override
